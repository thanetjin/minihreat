...
<div class="grid grid-cols-3 gap-4 min-h-screen">
    ...
    <div class="col-span-2 block p-8 bg-white border border-gray-200 rounded-lg shadow h-full">
            ...
            <p class="border border-gray-300 rounded-lg bg-gray-200 shadow h-1/4 p-2 mb-2">{{ $event->event_content }}</p>
            ...
            <p class="border border-gray-300 rounded-lg bg-gray-200 shadow p-2 mb-2">{{ $event->users->count() }}</p>
            <p class="text-gray-500 mt-5 mb-2">งบประมาณ</p>
            <p class="border border-gray-300 rounded-lg bg-gray-200 shadow p-2 mb-2">{{ $event->event_money }}</p>        
        ...
        <div class="flex justify-end">
            ...
            <form action="{{route('admin.accept', ['event' => $event])}}" method="POST">
                @csrf
                <button type="submit" class="flex focus:outline-none text-black bg-yellow-300 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-semibold rounded-lg text-sm px-5 py-1 mr-2 mb-2 dark:focus:ring-yellow-900 my-10">
                    ยอมรับคำร้อง
                    <svg class="w-3 h-3 ml-3 mt-1 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9"/>
                    </svg>
